...
<div class="ms-page nk-list-page">
  <div class="ms-page-head">
    ...
    <div class="ms-page-actions">
      <a href="{{ route('admin.commissions.export', request()->query()) }}" class="ms-btn-secondary">
  ...
  <div class="stat-grid mb-4">
    <div class="stat-card">
      <div>
        ...
        <div class="stat-value" style="font-size:1.1rem;">Rp {{ number_format($stats['total_pending'] ?? 0, 0, ',', '.') }}</div>
    ...
    <div class="stat-card">
      <div>
        ...
        <div class="stat-value" style="font-size:1.1rem;">Rp {{ number_format($stats['total_unpaid'] ?? 0, 0, ',', '.') }}</div>
    ...
    <div class="stat-card">
      <div>
        ...
        <div class="stat-value" style="font-size:1.1rem;">Rp {{ number_format($stats['total_paid'] ?? 0, 0, ',', '.') }}</div>
  ...
  <div class="ms-panel mb-3">
    <div class="ms-panel-body" style="padding:14px 18px;">
      <form method="GET" action="{{ route('admin.commissions.index') }}" class="d-flex flex-wrap gap-2 align-items-end">
        <div>
        ...
        <div>
          ...
          <select name="status" class="nk-length-select" style="min-width:140px;" onchange="this.form.submit()">
            ...
            <option value="unpaid" {{ request('status')=='unpaid'?'selected':'' }}>Dikonfirmasi</option>
        ...
        <div style="display:flex;gap:6px;align-items:flex-end;">
          ...
          @if(request()->hasAny(['partner_id','month','year','status']))
          <a href="{{ route('admin.commissions.index') }}" class="ms-btn-secondary" style="height:34px;color:var(--red);">
  ...
  <div class="ms-panel mb-3">
    ...
    <div class="table-responsive">
      <table class="table table-flat mb-0">
        ...
        <tbody>
          @foreach($partnerSummary as $ps)
          <tr>
            <td style="font-weight:600;">{{ $ps->user->name ?? '-' }}</td>
            <td class="text-end">{{ $ps->cnt }}</td>
            <td class="text-end" style="color:var(--blue);font-weight:600;">Rp {{ number_format($ps->unpaid_total,0,',','.') }}</td>
            <td class="text-end" style="color:var(--green);font-weight:600;">Rp {{ number_format($ps->paid_total,0,',','.') }}</td>
            <td class="text-end" style="font-weight:700;">Rp {{ number_format($ps->total,0,',','.') }}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
      ...
      <div class="table-responsive">
        <table class="table table-flat mb-0" id="commissions-table" style="min-width:1320px;">
          ...
          <tbody>
            ...
            <tr>
              <td style="font-weight:600;">{{ $commission->user->name ?? '-' }}</td>
              <td>{{ $commission->customer->name ?? '-' }}</td>
              <td>
                <div style="font-weight:600;">{{ $monthNames[$commission->month] ?? '-' }} {{ $commission->year }}</div>
                @if($commission->invoice)
                <div style="font-size:.72rem;color:var(--txt-3);">{{ $commission->invoice->invoice_number }}</div>
                @endif
              </td>
              <td>Rp {{ number_format($commission->customer->package_price ?? 0, 0, ',', '.') }}</td>
              <td style="font-weight:700;color:var(--green);">Rp {{ number_format($commission->amount, 0, ',', '.') }}</td>
              <td>
              ...
              <td style="font-size:.75rem;color:var(--txt-3);">
                ...
                <div><i class='bx bx-check-double' style="color:var(--green);"></i> Dibayar {{ $commission->paid_at?->format('d M Y') }}</div>
                @if($commission->payment_method)
                <div>Via: {{ $commission->payment_method }}</div>
                @endif
                @if($commission->payment_proof)
                <a href="{{ Storage::url($commission->payment_proof) }}" target="_blank" style="color:var(--blue);font-size:.72rem;"><i class='bx bx-file'></i> Lihat bukti</a>
                @endif
                @endif
              </td>
              <td>
                ...
                <button type="button" class="nk-action-btn pay" title="Cairkan Komisi"
                  ...
                  data-name="{{ $commission->user->name ?? '-' }}"
                  data-amount="Rp {{ number_format($commission->amount, 0, ',', '.') }}"
                  data-url="{{ route('admin.commissions.pay-single', $commission) }}"
