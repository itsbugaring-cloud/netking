        ...
        <tr>
          ...
          <td>
            ...
            <form method="POST" action="{{ route('admin.commissions.pay', $commission) }}" style="display:inline;">
              @csrf
              @php $commAmt = number_format($commission->amount, 0, ',', '.'); @endphp
              <button type="submit" class="btn btn-sm btn-primary" onclick="return confirm('Cairkan komisi Rp {{ $commAmt }} ke {{ $commission->user->name ?? '-' }}?')">
        ...
        <tr>
          <td colspan="8">
            <div class="empty-state">
              <div class="empty-state-icon"><i class='bx bx-dollar-circle'></i></div>
              <div class="empty-state-title">Belum ada komisi</div>
              <div class="empty-state-desc">Komisi otomatis dibuat saat invoice bulanan di-generate</div>
            </div>
          </td>
        </tr>
        @endforelse
