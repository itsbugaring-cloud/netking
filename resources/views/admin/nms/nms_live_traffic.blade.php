@extends('layouts.app')
@section('title', 'NMS Live Traffic')
...
<div id="areaGrid" class="row g-3">
    ...
    <div class="col-xl-4 col-lg-6" id="card-area-{{ $area->id }}">
        <div class="card h-100">
            <div class="card-body p-3">
                <div class="d-flex align-items-start justify-content-between mb-2">
                    <div>
                        <div style="font-weight:700;font-size:.9rem;">{{ $area->name }}</div>
                        <code style="font-size:.72rem;color:var(--text-muted);">{{ $area->router_ip }}</code>
                    </div>
                    <span class="area-status-badge badge" id="badge-{{ $area->id }}"
                ...
                <!-- Identity -->
                <div id="identity-{{ $area->id }}" style="font-size:.75rem;color:var(--text-muted);margin-bottom:.6rem;">
                ...
                <div class="d-flex align-items-center gap-2 mb-2">
                    ...
                    <span id="sessions-{{ $area->id }}" style="font-weight:700;font-size:.9rem;">—</span>
                    <button class="btn btn-sm sessions-btn ms-auto" data-area="{{ $area->id }}"
                ...
                <div id="traffic-{{ $area->id }}">
                    ...
                    <div class="mb-2">
                        <div class="d-flex justify-content-between" style="font-size:.72rem;margin-bottom:.2rem;">
                            ...
                            <span id="rx-label-{{ $area->id }}" style="font-weight:700;color:#3b82f6;">— Mbps</span>
                        </div>
                        <div style="background:#e5e7eb;border-radius:4px;height:8px;overflow:hidden;">
                            <div id="rx-bar-{{ $area->id }}" style="height:100%;width:0%;background:linear-gradient(90deg,#3b82f6,#60a5fa);border-radius:4px;transition:width .8s ease;"></div>
                        </div>
                    </div>
                    <!-- TX -->
                    <div class="mb-2">
                        <div class="d-flex justify-content-between" style="font-size:.72rem;margin-bottom:.2rem;">
                            ...
                            <span id="tx-label-{{ $area->id }}" style="font-weight:700;color:#8b5cf6;">— Mbps</span>
                        </div>
                        <div style="background:#e5e7eb;border-radius:4px;height:8px;overflow:hidden;">
                            <div id="tx-bar-{{ $area->id }}" style="height:100%;width:0%;background:linear-gradient(90deg,#8b5cf6,#a78bfa);border-radius:4px;transition:width .8s ease;"></div>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:.3rem;">
                        ...
                        <canvas id="spark-{{ $area->id }}" height="30" style="width:100%;display:block;border-radius:3px;"></canvas>
                    </div>
                </div>

                <!-- Interface breakdown toggle -->
                <div style="margin-top:.4rem;">
                    <button class="iface-toggle btn btn-sm" data-id="{{ $area->id }}"
                    ...
                    <div id="iface-breakdown-{{ $area->id }}" style="display:none;margin-top:.4rem;max-height:200px;overflow-y:auto;"></div>
                </div>

                <!-- Offline overlay -->
                <div id="offline-{{ $area->id }}" style="display:none;text-align:center;padding:.5rem 0;color:var(--red,#ef4444);font-size:.8rem;">
                    <i class='bx bx-wifi-off me-1'></i>Router Offline
                    <div id="offline-err-{{ $area->id }}" style="font-size:.68rem;color:var(--text-muted);margin-top:.2rem;"></div>
                </div>
            </div>
            ...
            <div class="card-footer" style="font-size:.68rem;color:var(--text-muted);padding:.4rem .75rem;display:flex;justify-content:space-between;">
                <span id="iface-{{ $area->id }}"><i class='bx bx-cable me-1'></i>—</span>
                <span id="ts-{{ $area->id }}">—</span>
            </div>
        </div>
    </div>
    @endforeach
...
<script>
const LIVE_DATA_URL   = '{{ route("admin.nms.live-traffic.data") }}';
const SESSIONS_URL    = '/admin/nms/live-traffic/'; // + areaId + '/sessions'
const SESSION_KILL_URL = '{{ route("admin.nms.session-kill") }}';
