...
<div class="ms-page-head">
    <div>
        ...
        <h1 class="ms-page-title">{{ $olt->name }}</h1>
        <div class="ms-page-subtitle">{{ $olt->brand }} {{ $olt->model }} · {{ $olt->ip_address }}</div>
    </div>
    <div class="ms-page-actions">
        ...
        <form action="{{ route('admin.olts.sync-now', $olt) }}" method="POST" class="d-inline"
        ...
        <a href="{{ route('admin.olts.edit', $olt) }}" class="ms-btn-ghost">
...
<div class="ms-inline-kpis mb-3">
    <a href="{{ route('admin.olts.index') }}" class="ms-kpi-chip" style="text-decoration:none;">
...
<div class="ms-stat-grid mb-4">
    <div class="ms-stat-card" style="--stat-accent:#2563eb;--stat-bg:#eff6ff;">
        ...
        <div>
            ...
            <div class="ms-stat-value">{{ $stats['total'] }}</div>
        </div>
    </div>
    <div class="ms-stat-card" style="--stat-accent:#22c55e;--stat-bg:#ecfdf3;">
        ...
        <div>
            ...
            <div class="ms-stat-value" style="color:var(--green);">{{ $stats['online'] }}</div>
        </div>
    </div>
    <div class="ms-stat-card" style="--stat-accent:#ef4444;--stat-bg:#fef2f2;">
        ...
        <div>
            ...
            <div class="ms-stat-value" style="color:var(--red);">{{ $stats['offline'] }}</div>
        </div>
    </div>
    <div class="ms-stat-card" style="--stat-accent:#f97316;--stat-bg:#fff7ed;">
        ...
        <div>
            ...
            <div class="ms-stat-value">{{ $stats['linked'] }}</div>
        </div>
    </div>
    <div class="ms-stat-card" style="--stat-accent:#a855f7;--stat-bg:#faf5ff;">
        ...
        <div>
            ...
            <div class="ms-stat-value" style="color:#a855f7;">{{ $stats['unlinked'] }}</div>
        </div>
    </div>
    <div class="ms-stat-card" style="--stat-accent:#0f766e;--stat-bg:#f0fdfa;">
        ...
        <div>
            ...
            <div class="ms-stat-value" style="font-size:1rem;">{{ strtoupper($olt->preferred_protocol) }}</div>
            <div class="ms-stat-meta">{{ $olt->ip_address }}</div>
        </div>
    </div>
</div>
...
<div class="ms-panel">
    <div class="ms-panel-head d-flex justify-content-between align-items-center">
        <span class="ms-panel-title">
            ...
            <span class="ms-2 ms-kpi-chip"><strong>{{ $stats['total'] }}</strong> devices</span>
        </span>
        ...
        <table class="table table-flat mb-0" id="ont-table">
            ...
            <tbody>
                ...
                <tr class="{{ $ont->status === 'offline' ? 'ont-offline-row' : '' }}"
                    data-status="{{ $ont->status ?? 'unknown' }}"
                    data-linked="{{ $ont->customer_id ? '1' : '0' }}">
                    <td class="cell-nowrap cell-index" style="font-size:.75rem;color:var(--text-muted);">{{ $loop->iteration }}</td>
                    <td class="cell-nowrap cell-serial">
                        <code style="font-size:.75rem;font-weight:600;white-space:nowrap;display:inline-block;">{{ $ont->serial_number }}</code>
                    </td>
                    ...
                    <td style="white-space:nowrap;">
                        ...
                        @if($q !== 'unknown')
                        <div style="font-size:.6rem;color:{{ $rxColor }};text-transform:uppercase;font-weight:600;letter-spacing:.4px;margin-top:1px;">{{ $q }}</div>
                        @endif
                        @else
                        ...
                        <form action="{{ route('admin.olts.link-customer', $ont) }}" method="POST" class="d-inline">
                            ...
                            <div class="d-flex gap-1 align-items-center">
                                <select name="customer_id" class="ont-link-select no-select2"
                                    ...
                                    <option value="{{ $c->id }}">{{ $c->name }}{{ $c->pppoe_user ? ' ('.$c->pppoe_user.')' : '' }}</option>
                                    @endforeach
                                </select>
                    ...
                    <td>
                        <div class="ont-row-actions">
                            <form action="{{ route('admin.olts.reboot-ont', $ont) }}" method="POST">
                                @csrf
                                <button type="submit" class="ont-action-btn"
                                    onclick="return confirm('Reboot ONT {{ $ont->serial_number }}?')">
                            ...
                            <button type="button" class="ont-action-btn"
                                ...
                                data-wan-url="{{ route('admin.olts.wan-config', $ont) }}"
                                data-ont-sn="{{ $ont->serial_number }}"
                                data-ont-pon="{{ $ont->pon_port }}:{{ $ont->olt_port_index }}">
                                <i class='bx bx-network-chart'></i> WAN
                            </button>

                            <button type="button" class="ont-action-btn"
                                onclick="return nkOpenProfileFromTrigger(this);"
                                data-profile-url="{{ route('admin.olts.set-profile', $ont) }}"
                                data-ont-sn="{{ $ont->serial_number }}"
                                data-ont-pon="{{ $ont->pon_port }}:{{ $ont->olt_port_index }}">
                            ...
                            @if($ont->customer_id)
                            <form action="{{ route('admin.olts.link-customer', $ont) }}" method="POST">
                                @csrf
...
{{-- ═══════════════════════════════════════════════════════════════════════
     Shared WAN Config Modal — outside the table, populated by JS
════════════════════════════════════════════════════════════════════════ --}}
@php $brand = strtolower($olt->brand); @endphp
                ...
                <div class="modal-header">
                    <div>
                        ...
                        <small>
                            ...
                            &nbsp;|&nbsp; <span style="text-transform:uppercase;font-weight:700;letter-spacing:.3px;">{{ $olt->brand }}</span>
                        </small>
                    </div>
                ...
                <div class="modal-body" style="padding:1.25rem;">
                    ...
                    <div class="row g-3 mb-1">
                        <div class="col-sm-4">
                            ...
                            <select name="wan_index" class="form-select form-select-sm">
                                @foreach(range(0,63) as $wid)
                                <option value="{{ $wid }}" {{ $wid === 0 ? 'selected' : '' }}>{{ $wid }}</option>
                                @endforeach
                            </select>
                            ...
                            <div class="col-sm-4">
                                ...
                                <select name="vlan_priority" class="form-select form-select-sm">
                                    ...
                                    <option value="{{ $p }}" {{ $p === 0 ? 'selected' : '' }}>
                                        {{ $p }}{{ $p === 0 ? ' — Best Effort' : ($p === 7 ? ' — Tertinggi' : '') }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="nk-modal-section">
                        ...
                        <div class="row g-3 mb-1">
                            ...
                            @if(str_contains($brand, 'cdata') || str_contains($brand, 'c-data'))
                            <div class="col-sm-3">
                            ...
                            @elseif(str_contains($brand, 'tenda'))
                            <div class="col-sm-4">
                                <label class="form-label">GEM Port</label>
                                <input type="number" name="gem_port" class="form-control form-control-sm"
                                    value="1" min="1" max="8">
                    ...
                    {{-- ─── Section 5: DBA Profile ──────────────────────────────── --}}
                    @if(!str_contains($brand, 'hsgq'))
                    <div class="nk-modal-section">
                        <div class="nk-modal-section-title">
...
{{-- ═══════════════════════════════════════════════════════════════════════
     Shared Set Profile Modal — outside the table, populated by JS
════════════════════════════════════════════════════════════════════════ --}}
        ...
        <form id="profileForm" action="" method="POST">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" style="padding:1.25rem;">
                    ...
                    @if(str_contains($brand,'cdata') || str_contains($brand,'c-data') || str_contains($brand,'hsgq'))
                    <div class="nk-modal-section">
