    ...
    <div class="col-md-4">
      <div class="ms-panel h-100">
        <div class="ms-panel-body d-flex align-items-center gap-3">
          <div style="width:44px;height:44px;background:{{ $connected ? '#f0fdf4' : '#fef2f2' }};border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <i class='bx {{ $connected ? "bxl-whatsapp" : "bx-wifi-off" }}' style="font-size:1.35rem;color:{{ $connected ? '#22c55e' : '#ef4444' }};"></i>
          </div>
          <div>
            <div style="font-weight:700;color:{{ $connected ? '#166534' : '#991b1b' }};">{{ $connected ? 'Terhubung' : 'Tidak Terhubung' }}</div>
            <div style="font-size:.8rem;color:#64748b;">{{ $configured ? 'Fonnte API key terkonfigurasi' : 'Tidak ada API key yang dikonfigurasi' }}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="row g-3">
    <div class="col-xl-6">
        ...
        <form method="POST" action="{{ route('admin.whatsapp.config') }}">
          ...
          <div class="ms-panel-body">
            <div class="mb-3">
              ...
              <input type="text" name="fonnte_api_key" class="form-control font-monospace" placeholder="Tempel token Fonnte Anda di sini" value="{{ config('services.fonnte.api_key') ? '●●●●●●●●●●●●●●●●' : '' }}">
      ...
      <div class="ms-panel mt-3">
        ...
        <form method="POST" action="{{ route('admin.whatsapp.test-send') }}">
          ...
          <div class="ms-panel-foot d-flex justify-content-end">
            <button type="submit" class="ms-btn" {{ !$configured ? 'disabled' : '' }}>
    ...
    <div class="col-xl-6">
      <div class="ms-panel">
        ...
        <form method="POST" action="{{ route('admin.whatsapp.send-customer') }}">
          @csrf
          <div class="ms-panel-body">
            <div class="mb-3">
