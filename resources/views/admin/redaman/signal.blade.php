...
<div class="olt-stack collapsed" id="olt-stack-{{ $olt->id }}">
  {{-- OLT Header (clickable accordion) --}}
  <div class="olt-header" onclick="toggleOlt({{ $olt->id }})">
    <span class="olt-dot {{ $oltStatus }}"></span>
    <div style="min-width:0;">
      <div class="olt-name">{{ $olt->name }}</div>
      <div class="olt-meta">
        {{ trim($olt->brand.' '.$olt->model) ?: 'OLT' }}
        @if($olt->ip_address) &bull; {{ $olt->ip_address }} @endif
        @if($olt->area) &bull; {{ $olt->area->name }} @endif
      </div>
    </div>
    <div class="olt-badges">
      @if($oltGood)   <span class="olt-badge badge-good">✓ {{ $oltGood }} Baik</span>   @endif
      @if($oltWeak)   <span class="olt-badge badge-weak">⚠ {{ $oltWeak }} Lemah</span>  @endif
      @if($oltCrit)   <span class="olt-badge badge-crit">✕ {{ $oltCrit }} Kritis</span> @endif
      @if($oltNoData) <span class="olt-badge badge-nodata">— {{ $oltNoData }} No Data</span> @endif
      <span class="olt-badge badge-total">{{ $oltTotal }} ONT</span>
    </div>
  ...
  <div class="olt-body">
    ...
    <table class="ont-table">
      ...
      <tr class="ont-row {{ $fqClass }}"
          data-ont-id="{{ $ont->id }}"
          data-customer-id="{{ $custId }}"
          data-cust-name="{{ strtolower($custName) }}"
          data-pppoe="{{ strtolower($pppoeUser) }}"
          data-serial="{{ strtolower($serial) }}"
          data-quality="{{ $quality }}"
          onclick="toggleOntDetail({{ $ont->id }})">
        <td style="color:var(--text-muted,#6b7280);font-size:.75rem;text-align:center;">{{ $idx+1 }}</td>
        <td>
          <div style="font-weight:600;white-space:nowrap;">{{ $custName }}</div>
          @if($ont->area)<div style="font-size:.7rem;color:var(--text-muted,#6b7280);">{{ $ont->area->name }}</div>@endif
        </td>
        <td><span style="font-family:monospace;font-size:.8rem;">{{ $pppoeUser }}</span></td>
        <td><span style="font-family:monospace;font-size:.78rem;color:var(--text-muted,#6b7280);">{{ $serial }}</span></td>
        <td style="font-size:.78rem;white-space:nowrap;">{{ $portStr }}</td>
        <td style="white-space:nowrap;">
          <span class="ont-status-dot {{ $ontStatus }}"></span>
          <span style="font-size:.78rem;text-transform:capitalize;">{{ $ontStatus }}</span>
        </td>
        <td onclick="event.stopPropagation()">
          <div style="display:flex;align-items:center;gap:8px;white-space:nowrap;" id="rx-cell-{{ $ont->id }}">
            @if($rxPower !== null)
              <span style="font-family:monospace;font-weight:700;font-size:.85rem;">{{ number_format($rxPower,2) }} dBm</span>
              <div class="sig-bar-wrap"><div class="sig-bar-fill" style="width:{{ $barPct }}%;background:{{ $barColor }};"></div></div>
            @else
        ...
        <td onclick="event.stopPropagation()">
          <span class="sig-badge {{ $quality }}" id="qbadge-{{ $ont->id }}">{{ $qLabel[$quality] }}</span>
        </td>
        <td style="text-align:center;" onclick="event.stopPropagation()">
          @if($custId)
            <button class="btn-live" id="btn-live-{{ $ont->id }}"
                    onclick="checkLive({{ $custId }}, {{ $ont->id }}, this)">
      ...
      {{-- Detail expand row --}}
      <tr class="ont-detail-row" id="detail-row-{{ $ont->id }}">
        <td colspan="9" class="ont-detail-cell">
          <div class="ont-detail-grid">
            <div class="ont-detail-item">
              <div class="lbl">Deskripsi / Model</div>
              <div class="val">{{ $ont->description ?: ($ont->equipment_id ?: '—') }}</div>
            </div>
            <div class="ont-detail-item">
              <div class="lbl">Firmware</div>
              <div class="val">{{ $ont->firmware_version ?: '—' }}</div>
            </div>
            <div class="ont-detail-item">
              <div class="lbl">Tx Power</div>
              <div class="val">{{ $ont->tx_power !== null ? number_format($ont->tx_power,2).' dBm' : '—' }}</div>
            </div>
            <div class="ont-detail-item">
              <div class="lbl">Jarak</div>
              <div class="val">{{ $ont->distance ? number_format($ont->distance,0).' m' : '—' }}</div>
            </div>
            <div class="ont-detail-item">
              <div class="lbl">Last Sync</div>
              <div class="val">{{ $ont->last_synced_at ? $ont->last_synced_at->diffForHumans() : '—' }}</div>
            </div>
          </div>
          {{-- Live ACS data (filled by JS) --}}
          <div id="live-detail-{{ $ont->id }}" style="margin-top:10px;display:none;"></div>
        </td>
      </tr>
      @endforeach
      </tbody>
...
<div style="text-align:center;padding:60px 20px;color:var(--text-muted,#6b7280);">
  ...
  <br><a href="{{ route('admin.olts.index') }}" style="margin-top:12px;display:inline-block;">Tambah OLT →</a>
</div>
@endforelse
...
<div style="margin-top:30px;border:1px solid var(--border-color,#e5e7eb);border-radius:10px;overflow:hidden;background:var(--card-bg,#fff);">
  ...
  <div style="display:flex;flex-wrap:wrap;">
    ...
    <div style="flex:1;min-width:180px;padding:10px 16px;border-right:1px solid var(--border-color,#e5e7eb);">
      <span class="sig-badge {{ $ref[1] }}" style="font-size:.7rem;">{{ $ref[2] }}</span>
      <div style="font-size:.78rem;font-weight:600;margin:4px 0 2px;font-family:monospace;">{{ $ref[0] }}</div>
      <div style="font-size:.72rem;color:var(--text-muted,#6b7280);">{{ $ref[3] }}</div>
    </div>
    @endforeach
  </div>
</div>
...
async function checkLive(customerId, ontId, btn) {
  ...
  try {
    const res  = await fetch(`{{ url('/admin/signal/check') }}/${customerId}`, {
