...
<?php $__env->startSection('title', 'Pelanggan: ' . $customer->name); ?>
...
<div class="ms-page-head">
    <div>
        ...
        <h1 class="ms-page-title"><?php echo e($customer->name); ?></h1>
    </div>
    <div class="ms-page-actions">
        ...
        <a href="<?php echo e(route('admin.customers.edit', $customer)); ?>" class="ms-btn">
            <i class='bx bx-edit'></i> Ubah
        </a>
        <?php endif; ?>
        <a href="<?php echo e(route('admin.customers.index')); ?>" class="ms-btn-secondary">
...
<?php if($customer->pppoe_pending_enable): ?>
<div class="alert mb-3" style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;">
    <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div>
            ...
            <div style="font-size:.8rem;color:#c2410c;margin-top:2px;">
                Invoice sudah lunas tapi PPPoE <strong><?php echo e($customer->pppoe_user); ?></strong> gagal diaktifkan otomatis karena MikroTik tidak reachable saat konfirmasi pembayaran.
                <?php if($customer->error_message): ?>
                    <br>Alasan: <?php echo e($customer->error_message); ?>
    ...
    <?php if (! ($isFinance)): ?>
    <form action="<?php echo e(route('admin.customers.enable-pppoe', $customer)); ?>" method="POST" style="flex-shrink:0;">
        <?php echo csrf_field(); ?>
        <button type="submit" class="btn btn-sm btn-warning" style="border-radius:8px;font-weight:700;white-space:nowrap;"
            onclick="return confirm('Aktifkan PPPoE <?php echo e($customer->pppoe_user); ?> sekarang?')">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-body text-center py-4">
                <div class="avatar avatar-lg mb-3 mx-auto" style="width:72px; height:72px; font-size:1.75rem; background: hsl(<?php echo e(crc32($customer->name) % 360); ?>, 55%, 60%);">
                    <?php echo e(strtoupper(substr($customer->name, 0, 1))); ?>

                </div>
                <h5 style="font-weight:700; color:#1e293b; margin-bottom:0.25rem;"><?php echo e($customer->name); ?></h5>
                <div style="font-size:0.875rem; color:#64748b; margin-bottom:0.75rem;"><?php echo e($customer->phone ?? 'Tidak ada telepon'); ?></div>
                <?php
                ...
                ?>
                <span class="badge-status <?php echo e($sclass); ?>"><?php echo e($slabel); ?></span>
            </div>
            <div class="card-body" style="border-top:1px solid #f0eff5; padding-top:1rem;">
                ...
                ['Remote IP', "<code style='background:#f5f5f9;padding:2px 8px;border-radius:4px;font-size:0.8125rem;color:#2563eb;'>" . ($customer->remote_ip ?? 'Dinamis') . "</code> <button class='btn btn-sm btn-clipboard p-0 ms-1' data-clipboard-text='" . ($customer->remote_ip ?? ' Dinamis') . "' title='Salin' style='border:none;background:none;color:#94a3b8;cursor:pointer;'><i class='bx bx-copy'></i></button>" ],
                    ...
                    <div class="d-flex justify-content-between mb-2 pb-2" style="border-bottom:1px solid #f0eff5;">
                        <span style="font-size:0.75rem; color:#64748b; font-weight:600; text-transform:uppercase; letter-spacing:0.5px;"><?php echo e($field[0]); ?></span>
                        <span style="font-size:0.875rem; color:#1e293b;"><?php echo $field[1]; ?></span>
                    </div>
            ...
            <div class="card-footer" style="border-top:1px solid #dbdade; padding:1rem 1.5rem;">
                <form action="<?php echo e(route('admin.customers.toggle-status', $customer)); ?>" method="POST">
                    ...
                    <button type="submit" class="btn w-100 <?php echo e($customer->status === 'active' ? 'btn-warning' : 'btn-success'); ?>">
        ...
        <ul class="nav nav-tabs mb-0" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-equipment" type="button" role="tab">
                    ...
                    <span class="badge bg-primary-subtle text-primary ms-1" style="font-size:.65rem;"><?php echo e($customer->devices->count()); ?></span>
                </button>
            </li>
            <li class="nav-item" role="presentation">
                ...
                <div class="card" style="border-top-left-radius:0;border-top-right-radius:0;">
                    <div class="table-responsive">
                        <table class="table table-flat mb-0" id="customer-invoices-table">
                            ...
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $customer->invoices()->latest()->take(10)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr>
                                    <td>
                                        <a href="<?php echo e(route('admin.invoices.show', $invoice)); ?>" style="color:#2563eb; font-size:0.8125rem; font-family:monospace;">
                                            <?php echo e($invoice->invoice_number); ?>

                                        </a>
                                    </td>
                                    <td style="font-weight:600; color:#1e293b;">Rp <?php echo e(number_format($invoice->amount, 0, ',', '.')); ?></td>
                                    <td style="font-size:0.8125rem; color:#64748b;"><?php echo e(\Carbon\Carbon::parse($invoice->billing_month)->format('M Y')); ?></td>
                                    <td style="font-size:0.8125rem; color:#64748b;"><?php echo e($invoice->due_date?->format('d M Y')); ?></td>
                                    <td>
                                        <?php if($invoice->status === 'paid'): ?>
                                        <span class="badge-status badge-paid">Lunas</span>
                                        <?php elseif($invoice->status === 'unpaid' && $invoice->due_date?->isPast()): ?>
                                    ...
                                    <td>
                                        <a href="<?php echo e(route('admin.invoices.show', $invoice)); ?>" class="btn btn-sm" style="background:rgba(3,195,236,0.12); color:#03c3ec; width:28px; height:28px; display:flex; align-items:center; justify-content:center; border-radius:6px; padding:0;">
                ...
                <div class="card" style="border-top-left-radius:0;border-top-right-radius:0;">
                    ...
                    <div class="table-responsive">
                        <table class="table table-flat mb-0">
                            ...
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $customer->devices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $device): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr>
                                    <td>
                                        <span class="badge bg-primary-subtle text-primary" style="font-size:.7rem;"><?php echo e($device->type_label); ?></span>
                                    </td>
                                    <td style="font-size:.8125rem; color:#1e293b;">
                                        <?php echo e($device->brand ?? '—'); ?>

                                        <?php if($device->model): ?>
                                        <span style="color:#64748b;"><?php echo e($device->model); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if($device->serial_number): ?>
                                        <code style="background:#f5f5f9;padding:2px 6px;border-radius:4px;font-size:.75rem;color:#2563eb;"><?php echo e($device->serial_number); ?></code>
                                        <?php else: ?>
                                        <span style="color:#94a3b8;font-size:.8rem;">—</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo $device->status_badge; ?></td>
                                    <td style="font-size:.8rem;color:#64748b;"><?php echo e($device->assigned_at?->format('d M Y') ?? '—'); ?></td>
                                    <td>
                                        <?php if (! ($isFinance)): ?>
                                        <div class="d-flex gap-1">
                                            <button class="btn btn-sm px-1 py-0" style="background:rgba(37,99,235,.1);color:#2563eb;border-radius:4px;" title="Ubah"
                                                onclick="editDevice(<?php echo e(json_encode($device)); ?>)">
                                                <i class='bx bx-edit-alt' style="font-size:.8rem;"></i>
                                            </button>
                                            <form action="<?php echo e(route('admin.customers.devices.destroy', [$customer, $device])); ?>" method="POST"
...
'provisioned' => ['bx-check-circle', 'var(--green)'],
                                ...
                                ?>
                                <div style="width:28px;height:28px;border-radius:6px;background:<?php echo e($icon[1]); ?>15;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;">
                                    <i class='bx <?php echo e($icon[0]); ?>' style="font-size:.85rem;color:<?php echo e($icon[1]); ?>;"></i>
                                </div>
                                <div style="flex:1;min-width:0;">
                                    <div style="font-size:.8rem;font-weight:500;"><?php echo e($log->description); ?></div>
                                    <div style="font-size:.7rem;color:var(--text-muted);">
                                        <?php echo e($log->created_at->diffForHumans()); ?>

                                        <?php if($log->user): ?>
                                        · oleh <?php echo e($log->user->name); ?>
                                        ...
                                        <span class="d-block mt-1" style="font-size:.65rem;">
                                            ...
                                            <?php if(is_array($change)): ?>
                                                <?php echo e($change['old'] ?? '—'); ?> → <?php echo e($change['new'] ?? '—'); ?>
                            ...
                            <?php $__currentLoopData = $invoiceLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inv): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div style="display:flex;align-items:flex-start;gap:.75rem;padding:.625rem 1.25rem;border-bottom:1px solid var(--border);">
                                ...
                                <div style="flex:1;">
                                    <div style="font-size:.8rem;font-weight:500;">Tagihan <?php echo e($inv->invoice_number); ?> lunas — Rp <?php echo e(number_format($inv->amount, 0, ',', '.')); ?></div>
                                    <div style="font-size:.7rem;color:var(--text-muted);"><?php echo e($inv->paid_at->diffForHumans()); ?> · <?php echo e(\Carbon\Carbon::parse($inv->billing_month)->format('M Y')); ?></div>
                                </div>
                            </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                            <?php if($logs->isEmpty() && $invoiceLogs->isEmpty()): ?>
            ...
            <div class="tab-pane fade" id="tab-errors" role="tabpanel">
                <div class="card card-danger" style="border-top-left-radius:0;border-top-right-radius:0;">
                    ...
                    <div class="card-body">
                        <pre style="color:#ff3d00; background:#fff5f5; padding:1rem; border-radius:0.375rem; font-size:0.8125rem;"><?php echo e($customer->error_message); ?></pre>
                        <?php if($customer->status === 'failed' && ! $isFinance): ?>
                        <form action="<?php echo e(route('admin.customers.retry-provision', $customer)); ?>" method="POST" class="mt-2">
                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
...
<div class="modal fade" id="resetPortalPasswordModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.customers.reset-portal-password', $customer)); ?>" method="POST">
                <?php echo csrf_field(); ?>
...
<div class="modal fade" id="addDeviceModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?php echo e(route('admin.customers.devices.store', $customer)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="date" name="assigned_at" class="form-control" value="<?php echo e(date('Y-m-d')); ?>">
                        </div>
                        <div class="col-6">
...
<script>
    function loadTopology() {
        ...
        $.getJSON('<?php echo e(route("admin.customers.topology", $customer)); ?>')
            .done(function(d) {
                if (!d.acs && !d.ont) {
    ...
    function editDevice(device) {
        ...
        form.action = '<?php echo e(route("admin.customers.devices.store", $customer)); ?>'.replace('/devices', '/devices/' + device.id);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/netking.id/resources/views/admin/customers/show.blade.php ENDPATH**/ ?>